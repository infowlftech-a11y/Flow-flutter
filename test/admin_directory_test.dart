...
void main() {
  group('filterUsers', () {
    ...
    test('role and status filters slice, not search', () {
      expect(
        filterUsers(users, UserFilter.riders, '').map((u) => u.uid),
        ['u1', 'u4'],
    ...
    test('search is case-insensitive and matches name or email', () {
      ...
      expect(
        filterUsers(users, UserFilter.all, '  seif ').single.uid,
        'u1',
    ...
    test('search composes with the filter rather than replacing it', () {
      ...
      expect(
        filterUsers(users, UserFilter.suspended, 'seif'),
        isEmpty,
    ...
    test('P3 — the member ID finds its account, whole or partial', () {
      // u1 is a rider (FLW-R-U1), u2 sells hours (FLW-C-U2). Support reads
      // these off tickets in any case and often in part.
      expect(filterUsers(users, UserFilter.all, 'FLW-R-U1').single.uid, 'u1');
      expect(filterUsers(users, UserFilter.all, 'flw-c-u2').single.uid, 'u2');
      expect(
        filterUsers(users, UserFilter.all, 'flw-c').map((u) => u.uid),
        ['u2', 'u3'],
        reason: 'the C prefix alone slices to the coach side',
      );
    });
  });
  ...
  group('filterBookings', () {
    ...
    test('status filter is exact', () {
      expect(
        filterBookings(bookings, BookingStatus.confirmed, '').single.id,
        'b2',
      );
      expect(filterBookings(bookings, BookingStatus.inProgress, ''), isEmpty);
    });

    test('search matches either side of the booking', () {
      expect(
        filterBookings(bookings, null, 'karim').single.id,
        'b3',
        reason: 'by rider',
      );
      expect(
        filterBookings(bookings, null, 'youssef').single.id,
        'b3',
        reason: 'by trainer',
      );
    ...
    test('search matches the session reference tickets and reports carry', () {
      ...
      expect(
        filterBookings(bookings, null, 'flw-0829').length,
        4,
    ...
    test('status and search compose', () {
      expect(
        filterBookings(bookings, BookingStatus.completed, 'anna'),
        isEmpty,
        reason: "Anna's bookings are not completed",
      );
      expect(
        filterBookings(bookings, BookingStatus.completed, 'karim').single.id,
        'b3',
      );
    });
  });
