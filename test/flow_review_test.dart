...
void main() {
  ...
  group('§8.9 — when the composer appears', () {
    ...
    for (final s in [
      ...
      test('not for a ${s.wire} booking — only a session that happened',
          () async {
  ...
  group('§8.9 — once, and only once', () {
    test('the composer closes after the review lands', () async {
      final id = await seedBooking();
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 5, bookingId: id,
      );
    ...
    test('a second completed session re-opens it', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 5, bookingId: first,
      );
      final second = await seedBooking();

      expect((await reviewable())?.id, second,
          reason: 'each session earns its own review');
    });

    test('reviewing one trainer does not close the composer for another',
        () async {
      final a = await seedBooking();
      final b = await seedBooking(instructorId: 'trainer2');
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 5, bookingId: a,
      );
    ...
    test('another riders review does not close mine', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: 'rider2', userName: 'Other',
        rating: 1, bookingId: theirs,
      );
    ...
    test('submitting twice for the same booking writes one review', () async {
      ...
      for (var i = 0; i < 2; i++) {
        await repo.submit(
          trainerId: trainer, userId: rider, userName: 'Seif',
          rating: 5, comment: 'Great', bookingId: id,
        );
      }
    ...
    test('the second submit does not overwrite the first', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 5, comment: 'Brilliant', bookingId: id,
      );
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 1, comment: 'Changed my mind', bookingId: id,
      );

      final only = (await repo.watchForTrainer(trainer).first).single;
      expect(only.rating, 5);
      expect(only.comment, 'Brilliant',
  ...
  group('what submit writes', () {
    test('the canonical shape (§6.3)', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 4, comment: '  Solid coaching  ', bookingId: id,
      );
    ...
    test('a null comment becomes empty, never the string "null"', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 4, bookingId: id,
      );

      expect((await db.collection(Col.reviews).get()).docs.single.data()['comment'], '');
    });
    ...
    test('a rating above 5 is clamped, not rejected', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 99, bookingId: id,
      );

      expect((await repo.watchForTrainer(trainer).first).single.rating, 5,
          reason: 'firestore.rules rejects out-of-range writes outright, so '
  ...
  group('§8.10 — what it does to the score', () {
    Future<void> review(int stars, {String t = trainer, String by = rider}) async {
      final id = await seedBooking(instructorId: t, kiterId: by);
      await repo.submit(
        trainerId: t, userId: by, userName: by, rating: stars, bookingId: id,
      );
    }

    test('an unrated trainer shows 5.0 out of nothing', () async {
      expect(await rating(trainer), isNull,
          reason: 'absent from the map entirely — the UI substitutes '
              'RatingSummary.none');
    ...
    test('deleting a review returns the score to the others', () async {
      ...
      final one = (await repo.watchForTrainer(trainer).first)
          .firstWhere((r) => r.rating == 1);
    ...
    test('a review with no trainerId is skipped, not counted as ""', () async {
      ...
      await db.collection(Col.reviews).add({
        'userId': rider, 'rating': 5, 'comment': '', 'bookingId': 'x',
      });
  ...
  group('the trainers own review list', () {
    test('is newest first', () async {
      for (final (i, stars) in [3, 4, 5].indexed) {
        await db.collection(Col.reviews).add({
          'trainerId': trainer, 'userId': 'r$i', 'userName': 'R$i',
          'rating': stars, 'comment': '', 'bookingId': 'b$i',
    ...
    test('a review with no timestamp sorts last rather than crashing',
        () async {
      await db.collection(Col.reviews).add({
        'trainerId': trainer, 'userId': 'a', 'rating': 5, 'bookingId': 'b1',
        'createdAt': Timestamp.fromDate(DateTime(2026, 1, 1)),
      });
      await db.collection(Col.reviews).add({
        'trainerId': trainer, 'userId': 'b', 'rating': 3, 'bookingId': 'b2',
      });
    ...
    test('only that trainers reviews', () async {
      await db.collection(Col.reviews).add({
        'trainerId': 'trainer2', 'userId': 'a', 'rating': 1, 'bookingId': 'b1',
      });
  ...
  group('adversarial — eligibility is repository-deep only', () {
    test('submit does not verify the booking exists', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 5, bookingId: 'never-existed',
      );
    ...
    test('submit does not verify the reviewer was the rider', () async {
      ...
      await repo.submit(
        trainerId: trainer, userId: rider, userName: 'Seif',
        rating: 1, bookingId: id,
      );

      expect((await rating(trainer))!.average, 1.0,
          reason: 'the composer would never have opened, but the write '
              'itself is unguarded');
    });

    test('a completed booking still reviewable after the trainer is deleted',
        () async {
      final id = await seedBooking();
